...
#SBATCH --job-name=doc.mt.65536.v4.pretrain.ft.cb.0.5.full.diff.key.pos.win.2
...
slang=en
SAVE=${SAVE_ROOT}/mt_v5_sep_65536_4_win_2_cb_ft_w0.5_full_diff_key_seg_pos_base
mkdir -p ${SAVE}
...
python -u train.py ${DATA} \
     ...
     --input-form sep --context-form window --window-size 2 --context-compress '2,2' \
     --context-model-path ${vqvae_model_path} --encode-code 1 --use-seg-pos-emb 1 --ctx-value-weight 0.5 \
     --sep-attn-share-key-proj 0 --fix-code-book 0 --tokens-per-sample 10000000 \
     --label-smoothing 0.1 --max-tokens 10384 --code-extract-strategy full \
